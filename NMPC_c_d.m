...
for i = 1:N
    x_next = euler_robot((i - 1) * t_s, x(:, i), u(:, i), t_s * t_opt); % 积分器这边改成t_s, 相应t_horizon改成3就不对了，下面nmpc里的也要改
...
for i = 1:N_nmpc
    ...
    noise = normrnd(0, 0.01, 4, 1);
    x_real = euler_robot((i - 1) * t_s, x_real, u_, t_s * t_opt) + noise;
    
    opti.set_value(x_0, x_real);
